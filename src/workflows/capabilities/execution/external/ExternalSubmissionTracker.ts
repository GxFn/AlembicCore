...
export class ExternalSubmissionTracker {
  ...
  recordSubmission(dimId: string, submissionArgs: SubmissionArgs, recipeId: string) {
    let submissions = this.#dimensionSubmissions.get(dimId);
    if (!submissions) {
      submissions = [];
      this.#dimensionSubmissions.set(dimId, submissions);
    }
    ...
    for (const source of record.sources) {
      ...
      let dimensionIds = this.#fileEvidenceMap.get(filePath);
      if (!dimensionIds) {
        dimensionIds = new Set();
        this.#fileEvidenceMap.set(filePath, dimensionIds);
      }
  ...
  recordRejection(dimId: string, title: string, reason: string) {
    let rejections = this.#rejections.get(dimId);
    if (!rejections) {
      rejections = [];
      this.#rejections.set(dimId, rejections);
    }
    rejections.push(`${title}: ${reason}`);
