/**
 * CoverageLedgerWrite — U2 覆盖账本写入的共享适配层（纯输入版）。
 *
...
export function writeCoverageLedgerForCompletion(
  ...
  try {
    ...
    // 仅写「不在已建 cell 里」的格子，已建格是真实测量结果，不能被空 deferred 行盖掉。
 ...
 * 最后一次完成的 completedAt 即轮末。new_recipes 是收益递减判定（new_recipes < K）的真实输入。
 *
