...
export interface GenerateSnapshotEntity {
  id: string;
...
export interface GenerateSnapshotInsert {
  id: string;
  sessionId?: string | null;
  projectRoot: string;
  createdAt: string;
...
export class GenerateRepositoryImpl extends RepositoryBase<
  typeof generateSnapshots,
...
> {
  ...
  constructor(
    drizzle: ConstructorParameters<
      typeof RepositoryBase<typeof generateSnapshots, GenerateSnapshotEntity>
    >[0]
  ) {
    super(drizzle, generateSnapshots);
  }

  /* ─── CRUD ─── */

  async findById(id: string): Promise<GenerateSnapshotEntity | null> {
  ...
  async create(data: GenerateSnapshotInsert): Promise<GenerateSnapshotEntity> {
    ...
    if (!created) {
      throw new Error(`Bootstrap snapshot not found after insert: ${data.id}`);
    }
    return created;
  }
  ...
  /** 获取项目最新完成的快照 */
  async getLatest(projectRoot: string): Promise<GenerateSnapshotEntity | null> {
  ...
  /** 获取项目的快照列表 (按时间降序) */
  async listByProject(projectRoot: string, limit = 10): Promise<GenerateSnapshotEntity[]> {
    const rows = this.drizzle
      .select()
      .from(this.table)
  ...
  async enforceCapacity(
    projectRoot: string,
    maxSnapshots = GenerateRepositoryImpl.MAX_SNAPSHOTS
  ): Promise<number> {
  ...
  async saveWithDimFiles(
    snapshot: GenerateSnapshotInsert,
        ...
        .where(
          and(
            ...
            sql`${this.table.id} NOT IN (
              ...
              ORDER BY ${this.table.createdAt} DESC
              LIMIT ${GenerateRepositoryImpl.MAX_SNAPSHOTS}
            )`
          )
        )
        .run();
    ...
    const created = await this.findById(snapshot.id);
    if (!created) {
      throw new Error(`Bootstrap snapshot not found after insert: ${snapshot.id}`);
    }
    return created;
  }
  ...
  #mapRow(row: typeof generateSnapshots.$inferSelect): GenerateSnapshotEntity {
    return {
      id: row.id,
