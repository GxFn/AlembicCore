...
export class KnowledgeRepositoryImpl {
  ...
  _rowToEntity(row: Record<string, unknown>): KnowledgeEntry | null {
    ...
    return new KnowledgeEntry({
      ...
      retrievalProfile: safeJsonParse(row.retrievalProfile, null),
  ...
  _entityToRow(e: KnowledgeEntry) {
    ...
    return {
      ...
      retrievalProfile: e.retrievalProfile ? safeJsonStringify(e.retrievalProfile) : null,
  ...
  findByIdsDetailSync(ids: string[]) {
    ...
    return this.#drizzle
      ...
      .from(knowledgeEntries)
      .where(and(inArray(knowledgeEntries.id, ids), ne(knowledgeEntries.lifecycle, 'deprecated')))
      .all();
  }
