...
import { isExcludedProject } from './isOwnDevRepo.js';
...
class PathGuard {
  ...
  /** 宿主显式扩展的项目内写入前缀 */
  #extraProjectWritePrefixes = new Set<string>();

  /** 宿主显式扩展的项目根可写文件 */
  #extraProjectWritableFiles = new Set<string>();
  ...
  configure({
    ...
    knowledgeBaseDir,
    extraAllowPaths = [],
    extraProjectWritePrefixes = [],
    extraProjectWritableFiles = [],
  }: PathGuardConfigureOptions) {
    ...
    this.#extraProjectWritePrefixes.clear();
    this.#extraProjectWritableFiles.clear();
    ...
    for (const prefix of extraProjectWritePrefixes) {
      const normalized = normalizeProjectWritePrefix(prefix);
      if (normalized) {
        this.#extraProjectWritePrefixes.add(normalized);
      }
    }

    for (const file of extraProjectWritableFiles) {
      const normalized = normalizeProjectRootFile(file);
      if (normalized) {
  ...
  assertSafe(targetPath: string) {
    ...
    if (!targetPath || typeof targetPath !== 'string') {
      throw new PathGuardError(String(targetPath), projectRoot);
    }

    const resolved = path.resolve(targetPath);

    // 1. 项目目录内 — 允许
    if (this.#isUnder(resolved, projectRoot)) {
      return;
    }
    ...
    // 越界
    throw new PathGuardError(resolved, projectRoot);
  }
  ...
  assertProjectWriteSafe(targetPath: string) {
    ...
    if (this.#isExcludedProject) {
      ...
      if (this.#extraProjectWritableFiles.has(relative)) {
        return;
      }
      throw new PathGuardError(
        resolved,
    ...
    // 检查是否在允许的前缀中
    for (const prefix of this.#projectWriteScopePrefixes()) {
    ...
    if (
      PROJECT_ROOT_WRITABLE_FILES.includes(relative) ||
      this.#extraProjectWritableFiles.has(relative)
    ) {
      return;
    }
    ...
    throw new PathGuardError(
      resolved,
      projectRoot,
      `项目内写入范围受限: "${relative}" 不在允许的目录中（允许: ${[...this.#projectWriteScopePrefixes(), kbDir || 'Alembic'].join(', ')}）`
    );
  }
  ...
  addProjectWritePrefix(relativePrefix: string) {
    const normalized = normalizeProjectWritePrefix(relativePrefix);
    if (normalized) {
      this.#extraProjectWritePrefixes.add(normalized);
    }
  }
  ...
  #projectWriteScopePrefixes(): string[] {
    return [...PROJECT_WRITE_SCOPE_PREFIXES, ...this.#extraProjectWritePrefixes];
  }
}
...
function normalizeProjectWritePrefix(value: string): string {
  return normalizeProjectRelativePath(value).split(path.sep)[0] || '';
}

function normalizeProjectRootFile(value: string): string {
  const normalized = normalizeProjectRelativePath(value);
  if (!normalized || normalized.includes(path.sep)) {
    return '';
  }
  return normalized;
}
