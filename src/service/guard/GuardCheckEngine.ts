...
export class GuardCheckEngine {
  ...
  _runAstLayer2Checks(code: string, language: string, filePath: string): GuardViolation[] {
    ...
    const numericThreshold = (ruleId: string, fallback: number) => {
      const value = thresholds[ruleId];
      return typeof value === 'number' ? value : fallback;
    };
    const classBloatLimit = numericThreshold('ast_class_bloat', 30);
    const complexityLimit = numericThreshold('ast_method_complexity', 20);
    const methodLengthLimit = numericThreshold('ast_method_too_long', 120);
    const nestingLimit = numericThreshold('ast_deep_nesting', 6);
    const inheritanceDepthLimit = numericThreshold('ast_deep_inheritance', 4);
    const protocolConformanceLimit = numericThreshold('ast_wide_protocol_conformance', 5);
    const godClassMethodLimit = numericThreshold('ast_god_class_methods', 40);
    const godClassPropertyLimit = numericThreshold('ast_god_class_properties', 20);
