...
describe('ProjectContext PCQ-8 project space', () => {
  ...
  it('keeps remaining repos and returns an ordinary error for missing configured folders', async () => {
    await withFixture(
      {
        'workspace.config.json': workspaceConfig(['Existing', 'Missing']),
    ...
    await withFixture(
      {
        ...
        'workspace.config.json': JSON.stringify(
          {
            repoNames: ['A', 'B'],
            repositories: [
              { name: 'A', path: 'A', repositoryId: 'dup' },
              { name: 'B', path: 'B', repositoryId: 'dup' },
            ],
            workspaceName: 'DuplicateSpace',
          },
          null,
          2
        ),
...
function createWorkspaceFixture(): Record<string, string> {
  return {
    ...
    'workspace.config.json': workspaceConfig(['RepoA', 'RepoB']),
  };
}
  ...
  return JSON.stringify(
    {
      ...
      repositories: names.map((name) => ({
        name,
        path: name,
        repositoryId: `repo-${name.replace(/^Repo/, '').toLowerCase()}`,
      })),
      workspaceName: 'FixtureSpace',
    },
    null,
    2
  );
