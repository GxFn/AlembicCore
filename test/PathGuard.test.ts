...
describe('PathGuard', () => {
  ...
  describe('assertSafe (Layer 1 — boundary)', () => {
    ...
    test('should block null/undefined paths', () => {
      expect(() => pathGuard.assertSafe(null as unknown as string)).toThrow(PathGuardError);
      expect(() => pathGuard.assertSafe(undefined as unknown as string)).toThrow(PathGuardError);
    });
  ...
  describe('assertProjectWriteSafe (Layer 2 — project write scope)', () => {
    ...
    test('should block delivery adapter directories by default', () => {
      expect(() =>
    ...
    test('should allow delivery adapter directories when explicitly configured', () => {
      pathGuard.configure({
        projectRoot: PROJECT_ROOT,
        packageRoot: PACKAGE_ROOT,
        knowledgeBaseDir: 'Alembic',
    ...
    test('should block .env in project root by default', () => {
      expect(() => pathGuard.assertProjectWriteSafe(path.join(PROJECT_ROOT, '.env'))).toThrow(
        PathGuardError
      );
    });

    test('should allow adapter-owned root files when explicitly configured', () => {
      pathGuard.configure({
        projectRoot: PROJECT_ROOT,
        packageRoot: PACKAGE_ROOT,
        knowledgeBaseDir: 'Alembic',
        extraProjectWritableFiles: ['.env', 'nested/file.env'],
      });

      expect(() => pathGuard.assertProjectWriteSafe(path.join(PROJECT_ROOT, '.env'))).not.toThrow();
      expect(() =>
        pathGuard.assertProjectWriteSafe(path.join(PROJECT_ROOT, 'nested/file.env'))
      ).toThrow(PathGuardError);
    });
