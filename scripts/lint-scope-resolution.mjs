...
const GATED_DIRECTORIES = [
  ...
  'src/service/project-context/',
  'src/service/source-graph/',
];

const GATED_FILES = new Set(['src/shared/resolveProjectRoot.ts']);

const FROM_PROJECT_CALL =
  /\b(?:WorkspaceResolver|WR)\s*\.\s*fromProject\s*\(|\bfromProject\s*\(/;
const VALID_SINGLE_ROOT =
  /\/\/\s*@scope-singleroot\((?:permanent|temporary)\)\s+[—-]\s+\S/;
...
function checkFile(repoRoot, absolutePath) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (line.includes('fromProjectScopeRegistry') || line.includes('WorkspaceSettingsStore.fromProject')) {
      continue;
    }
    ...
    violations.push({
      line: index + 1,
      message: 'scan/write paths must use WorkspaceResolver.fromProjectScopeRegistry',
      repoRelativePath,
      snippet: line.trim(),
    });
  }
...
function main() {
  ...
  if (options.help) {
    console.log(`Usage: node scripts/lint-scope-resolution.mjs [--root path]

Checks Core scan/write paths for bare WorkspaceResolver.fromProject calls.
Use // @scope-singleroot(permanent|temporary) — reason for intentional single-root sites.`);
  ...
  if (violations.length === 0) {
    console.log('scope-resolution lint passed');
    return;
  }

  console.error('scope-resolution lint failed:');
